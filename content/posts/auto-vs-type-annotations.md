...
How is the type deduced? Using [template argument deduction](https://en.cppreference.com/w/cpp/language/template_argument_deduction#Other_contexts), which is quite similar to the process of solving for unknown variables in a set of mathematical equations. In the equation, $x+1=7$, it is easy to deduce that $x=6$. Likewise, in the expression `auto x = 3`, it's easy to deduce that the type of x is `integer`. Type deduction can get much more complicated, the study of which belongs to [type theory](https://en.cppreference.com/w/cpp/language/template_argument_deduction#Other_contexts). 
...
`var x = 3 (* no need for an 'int' keyword' *)`

Even in the case of functions with parameters, you can leave off the type annotations because the compiler is smart enough to figure out that the addition operator in Standard ML can only be used with `int`s, and the return type must be an `int` as well.
